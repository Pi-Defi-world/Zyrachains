...
router.get('/contact', auth_1.authenticateAdmin, async (req, res) => {
    try {
...
router.get('/contact/:id', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.patch('/contact/:id', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/report-scam', async (req, res) => {
    try {
        const { walletAddress, description, evidence, reporterContact, scamType, } = req.body;
...
router.get('/report-scam', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const status = req.query.status;
...
router.get('/report-scam/:id', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ error: 'Invalid ID format' });
...
router.patch('/report-scam/:id', auth_1.authenticateAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { status, priority, notes } = req.body;
        if (!mongoose_1.default.Types.ObjectId.isValid(id)) {
