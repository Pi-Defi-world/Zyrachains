...
import { buildCexFlows, buildHomePulse } from './snapshots';
import { cacheDel, cacheKeySegment } from './redisMemoryCache';
...
function leaseMs(): number {
  return Number(process.env.PCT_STREAM_LEASE_MS || 60_000);
}

function pollMs(): number {
  return Number(process.env.PCT_STREAM_POLL_MS || 15_000);
}

function backfillConcurrency(): number {
  return Math.max(1, Math.min(20, Number(process.env.PCT_STREAM_BACKFILL_CONCURRENCY || 6)));
}

function backfillBatchSize(): number {
  // Default higher so baseline becomes available quickly in dev.
  return Math.max(0, Math.min(2000, Number(process.env.PCT_STREAM_BACKFILL_BATCH_SIZE || 1000)));
}

function balanceRefreshIntervalMs(): number {
  return Math.max(60_000, Number(process.env.PCT_BALANCE_REFRESH_MS || 300_000));
}
  ...
  client.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (status === 429 || (status && status >= 500)) {
        ...
        if (retries > 3) return Promise.reject(error);
        const delay =
          status === 429
...
async function ensureState(): Promise<StreamState> {
  const col = mongoose.connection.collection<StreamState>('pct-stream-state');
  const existing = (await col.findOne({ _id: STREAM_ID })) as StreamState | null;
  if (existing) {
    // Migrate older docs to include progress/debug fields.
    await col.updateOne(
      { _id: STREAM_ID },
      [
        {
    ...
    return (await col.findOne({ _id: STREAM_ID })) as StreamState;
  }
...
async function releaseLease(): Promise<void> {
  const col = mongoose.connection.collection<StreamState>('pct-stream-state');
  await col.updateOne(
    { _id: STREAM_ID, leaseOwner: OWNER_ID },
    { $set: { leaseOwner: null, leaseExpiresAt: null } }
  );
}

async function loadTrackedWallets(): Promise<{
  coreSet: Set<string>;
  cexSet: Set<string>;
  ...
  const trackedSet = new Set<string>([...coreSet, ...cexSet]);
  return { coreSet, cexSet, trackedSet };
}

async function fetchBalance(identifier: string): Promise<number> {
  try {
    const { data } = await client.get<{ balances?: Array<{ asset_type: string; balance: string }> }>(
...
async function runBackfill(state: StreamState, coreSet: Set<string>, cexSet: Set<string>, trackedSet: Set<string>): Promise<void> {
  if (state.backfilledAt && process.env.PCT_STREAM_FORCE_BACKFILL !== 'true') return;
  const ids = [...trackedSet];
  const total = ids.length;
  const streamStateCol = mongoose.connection.collection<StreamState>('pct-stream-state');
  const stateCol = mongoose.connection.collection('pct-wallet-state');

  if (total === 0) {
    await streamStateCol.updateOne(
      { _id: STREAM_ID },
      { $set: { backfilledAt: new Date(), backfillTotal: 0, backfillIndex: 0 } }
    );
    return;
  }
  ...
  await streamStateCol.updateOne(
    { _id: STREAM_ID },
    { $set: { backfillTotal: total, backfillIndex: endIndex } }
  );

  if (batch > 0 && endIndex < total) return;
  ...
  const baselineTracked =
    typeof baselineTrackedAgg[0]?.total === 'number' ? baselineTrackedAgg[0].total : 0;
...
async function runBalanceRefresh(trackedSet: Set<string>): Promise<void> {
  const ids = [...trackedSet];
...
async function runOneCycle(): Promise<void> {
  ...
  try {
    const streamStateCol = mongoose.connection.collection<StreamState>('pct-stream-state');
    // Mark liveness immediately after acquiring lease.
    await streamStateCol.updateOne(
      { _id: STREAM_ID, leaseOwner: OWNER_ID },
    ...
    const { data } = await client.get<{ _embedded?: { records?: Array<Record<string, unknown>> } }>(
      '/payments',
      { params }
    );
    ...
    const stateOps: AnyBulkWriteOperation<any>[] = [];
    const movementOps: AnyBulkWriteOperation<any>[] = [];
    ...
    for (const row of records) {
      ...
      if (pagingToken) cursor = pagingToken;
      if (type !== 'payment' || assetType !== 'native' || amount <= 0) continue;
    ...
    if (eventDocs.length > 0) {
      ...
      if (cexEventCount > 0) {
        try {
          await buildCexFlows();
          await buildHomePulse();
          await Promise.all([
            cacheDel(cacheKeySegment('cex-flows')),
            cacheDel(cacheKeySegment('pulse')),
          ]);
        } catch (refreshErr) {
    ...
    const metaCol = mongoose.connection.collection<{ _id: string; lastFullScanAt?: Date; scanLock?: boolean }>(
      'pct-monitor-meta'
    );
    await metaCol.updateOne(
      { _id: 'meta' },
      { $set: { lastFullScanAt: new Date(), scanLock: false } },
      { upsert: true }
    );
    await refreshPctSummary();
  } catch (error) {
...
export function startPctStreamer(): void {
  if (process.env.PCT_STREAM_ENABLED !== 'true') return;
  if (timer) return;
  ...
  // Separate timer for balance refresh — doesn't block payment streaming
