...
export default function BoostModal({ post, onClose }: BoostModalProps) {
  ...
  const handleBoost = async () => {
    ...
    try { await boostPost(post._id, boostAmount); showToast(`Boosted with ${boostAmount} ZP!`, 'success'); onClose(); } catch (err: any) { setError(err.message || 'Boost failed'); } finally { setSubmitting(false); }
  };
      ...
      <div className="bg-card border border-border rounded-t-lg sm:rounded-lg w-full sm:max-w-sm shadow-md max-h-[85vh] overflow-y-auto animate-in slide-in-from-bottom sm:slide-in-from-bottom-0 duration-200">
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {BOOST_PRESETS.map((amt) => (<button key={amt} onClick={() => { setAmount(amt); setCustomAmount(''); }} className={`py-2.5 rounded text-xs font-medium transition-colors ${amount === amt && !customAmount ? 'bg-accent text-accent-foreground' : 'bg-secondary text-muted-foreground hover:bg-secondary/70'}`}>{amt} ZP</button>))}
          </div>
          <input type="number" value={customAmount} onChange={(e) => setCustomAmount(e.target.value)} placeholder="Custom amount..." min="10" step="1" className="w-full bg-secondary/30 border border-border rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
          ...
          <button onClick={handleBoost} disabled={submitting || (isNaN(boostAmount) || boostAmount < 10)} className="w-full mt-3 py-2.5 bg-accent text-accent-foreground rounded-md text-xs font-semibold hover:bg-accent/90 disabled:opacity-50 transition-colors">
