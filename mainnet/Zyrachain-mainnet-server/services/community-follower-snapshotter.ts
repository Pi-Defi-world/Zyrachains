...
/** Default: once every 30 days (2.59B ms). Override with COMMUNITY_SNAPSHOT_POLL_MS env. */
function pollMs(): number {
  return Math.max(300_000, Number(process.env.COMMUNITY_SNAPSHOT_POLL_MS || 2_592_000_000));
}
...
async function loadTrackedHandles(): Promise<{ handle: string; source: string; name: string }[]> {
  ...
  const result: { handle: string; source: string; name: string }[] = [];

  function add(h: string | null, source: string, name: string) {
  ...
  for (const doc of communityDocs) {
    add(normalizeTwitterHandle(doc.twitter), 'communitylisting', doc.name || '');
  }
  ...
  for (const doc of ecosystemDocs) {
    ...
    for (const k of ['Twitter', 'twitter', 'X', 'x', 'TwitterHandle', 'twitterHandle']) {
      ...
      if (typeof v === 'string') {
        ...
        if (h) { add(h, 'ecosystem-community', name); break; }
      }
    }
  ...
  for (const doc of influencerDocs) {
    add(normalizeTwitterHandle(doc.twitter), 'influencerlisting', doc.name || '');
  }
  ...
  for (const doc of ecosystemInfluencerDocs) {
    ...
    for (const k of ['Twitter', 'twitter', 'X', 'x', 'TwitterHandle', 'twitterHandle', 'XHandle', 'xHandle']) {
      ...
      if (typeof v === 'string') {
        const h = normalizeTwitterHandle(v);
        if (h) { add(h, 'ecosystem-influencer', name); break; }
      }
    }
    for (const k of ['Website', 'website', 'Link', 'link']) {
      const v = doc[k];
...
async function runOneCycle(): Promise<void> {
  try {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const r = await fetchTwitterPublicStats(entry.handle);
      if (!r.ok) {
      ...
      const snapshot = new CommunityFollowerSnapshot({
...
export function startCommunityFollowerSnapshotter(): void {
