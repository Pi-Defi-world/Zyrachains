...
async function getCexSummary() {
  ...
  const cexIds = await cexCol
    ...
    .toArray();
  const ids = cexIds.map((d) => d.identifier).filter(Boolean) as string[];
  ...
  const [sumAgg, net24, confirmedChanges] = await Promise.all([
    ...
    eventsCol
      .aggregate([
        { $match: { detectedAt: { $gte: new Date(Date.now() - 86_400_000) }, wallet: { $in: ids } } },
  ...
  return {
    ...
    walletsTrackedCoreTeam: null as number | null,
    scannedWallets,
    ...
    confirmedChanges,
    totalOut:
      typeof meta?.baselineSumPiCex === 'number' && typeof sumAgg[0]?.total === 'number'
...
router.get('/movements', async (req: Request, res: Response) => {
  try {
    ...
    const filter: Record<string, unknown> = { wallet: { $in: ids } };
...
/**
 * Balance history for a specific CEX wallet address
 */
...
/**
 * Aggregate balance history for all CEX wallets
 */
router.get('/aggregate-balance-history', async (req: Request, res: Response) => {
  try {
    ...
    // Reconstruct balance from change field
    const stateCol = mongoose.connection.collection('pct-wallet-state');
    ...
    for (const doc of docs) {
      ...
      if (typeof runningBalance !== 'number') continue;
